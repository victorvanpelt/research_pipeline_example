...
# Counted here rather than typed, so the sentence below cannot go stale.
n_missing  <- sum(is.na(data))
missing_txt <- if (n_missing == 0) "No variable has missing values." else
  sprintf("The data contain %d missing values.", n_missing)
```
...
The unit of observation is one synthetic firm-observation per row. The raw file contains `r n_raw` observations; the analysis data contain `r n_analysis`, after dropping observations with negative scaled earnings. `r missing_txt`
